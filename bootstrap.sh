...
if [ -z "$(docker compose run --rm openvpn sh -c 'ls /etc/openvpn/pki 2>/dev/null' < /dev/null)" ]; then
  ...
  source .env
  DETECTED_IP=""
  command -v curl > /dev/null 2>&1 && DETECTED_IP=$(curl -s --max-time 3 https://ifconfig.me || true)
  while [ -z "$SERVER_ADDR" ] || [ "$SERVER_ADDR" = "seu-ip-ou-dominio-aqui" ]; do
    if [ -n "$DETECTED_IP" ]; then
      read -r -p "IP público ou domínio desta VPS [Enter usa $DETECTED_IP]: " SERVER_ADDR
